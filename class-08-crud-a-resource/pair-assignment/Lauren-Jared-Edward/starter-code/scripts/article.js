(function(module) {
  ...
  Article.createTable = function(callback) {
    webDB.execute(
      'CREATE TABLE articles(title VARCHAR(50), category VARCHAR(50), author VARCHAR(50), authorUrl VARCHAR(50), publishedOn DATE, body VARCHAR(500);)',
      function(result) {
  ...
  Article.prototype.deleteRecord = function(callback) { //delete a specific id instance
    webDB.execute(
      [
        {
  ...
  Article.prototype.updateRecord = function(newa, callback) {
    webDB.execute(
      [
        {
          'sql': 'UPDDATE articles SET ? = ? WHERE ? = ?;',
          // 'data': []
        }
      ],
      callback
  ...
  Article.fetchAll = function(next) {
    webDB.execute('SELECT * FROM article;', function(rows) {
      if (rows.length) {
        ...
        Article.loadAll(rows); //not tested!!!!
      } else {
